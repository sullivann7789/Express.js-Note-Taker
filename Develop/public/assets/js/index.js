...
let handleNoteDelete = (e) => {
  ...
  let note = e.target;
  let noteId = JSON.parse(note.parentElement.getAttribute('data-note')).id;
...
const saveNote = (note) =>
 ...
 saveNoteBtn.addEventListener('click', () => saveNote().then((data) => renderActiveNote(db.json())));
const deleteNote = (id) =>
  fetch(`/notes/${id}`, {
    method: 'DELETE',
    headers: {
...
const renderNoteList = async (notes) => {
  let jsonNotes = await JSON.stringify(notes)
  if (window.location.pathname === '/notes') {
    noteList.forEach((el) => (el.innerHTML = ''));
  }
